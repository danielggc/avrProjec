...
void nRF24::mirf_config(uint8_t channel, uint8_t pay_length){
    ...
    mirf_config_register(RX_ADDR_P0,mirf_PAYLOAD);
    PTX =0;
    RX_POWERUP;
    mirf_CE_hi;
    */
...
uint8_t nRF24::nrf24_getStatus(){
    uint8_t testiando;    
    mirf_CE_lo;
    comunicacion.SPI_uint8_tTransmit(NOP);
    mirf_CE_hi;
    return testiando;
}
...
//comunicacion
void nRF24::mirf_send(uint8_t * value, uint8_t len){
        mirf_CE_lo;
        nrf24_powerUpTx();
        PTX=1;
        mirf_CSN_lo;
...
void nRF24::nrf24_transferSync(uint8_t* dataout,uint8_t* datain,uint8_t len){
    uint8_t i;
    for(i=0;i<len;i++){
        dataout[i] =comunicacion.SPI_uint8_tTransmit(dataout[i]);
    }
}
